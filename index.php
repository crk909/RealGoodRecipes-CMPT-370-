<!--Still requires a lot of work for functionality and design-->
<!---->
<!--Todo --->
<!--Javascript for inputting actual information from user specified books-->
<!--Dropdown for selecting books-->
<!--Button for creating new recipeBook?-->
...
require_once "Interfaces/RecipeBook.php";
...
if($_SERVER["REQUEST_METHOD"] == "GET"){
    ...
    //2nd exists for testing, should eventually be removed
    $displayID = $_SESSION["id"];
//    $displayID =1;


    $recipeBooktoDisplay = new RecipeBook($displayID);
    $IDstoDisplay = $recipeBooktoDisplay->getRecipeList();
...
<script>
    function fillRecipeThumbs(){
        ...
        var numRecs = thumbnailContainer.childElementCount;
        if(numRecs == 0){
            var noRecsMessage = document.createElement("p");
            noRecsMessage.innerHTML = "You have no liked recipes, either create your own or add them by searching!";

            thumbnailContainer.appendChild(noRecsMessage);
        }
